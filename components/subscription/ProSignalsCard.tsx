...
interface ProSignalsCardProps {
  signal: SignalRow;
}

/**
 * Pro Plan Signal Card - Includes expected range
 * Shows bias, entry price, conditions, expected range for stocks
 */
export function ProSignalsCard({ signal }: ProSignalsCardProps) {
  ...
  const expectedRange = (signal as any).expectedRange;
  const rangeLow = expectedRange?.low || "Calculating...";
  const rangeHigh = expectedRange?.high || "Calculating...";
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-500/30">
              <span className="text-lg font-bold text-purple-300">{signal.symbol}</span>
            </div>
            <div>
      ...
      <div className="px-6 py-5 space-y-4">
        ...
        <div className="mt-4 pt-4 border-t border-zinc-700/50">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-semibold text-purple-300 uppercase tracking-wider">Expected Range</span>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-zinc-800/50 border border-zinc-700 px-4 py-3">
              <div className="text-xs text-zinc-500 uppercase tracking-wider mb-1">Lowest</div>
              <div className="text-lg font-semibold text-rose-400">{rangeLow}</div>
            </div>
            <div className="rounded-lg bg-zinc-800/50 border border-zinc-700 px-4 py-3">
              <div className="text-xs text-zinc-500 uppercase tracking-wider mb-1">Highest</div>
              <div className="text-lg font-semibold text-emerald-400">{rangeHigh}</div>
            </div>
          </div>
        </div>
      </div>
