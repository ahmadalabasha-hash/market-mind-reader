...
import { type FormEvent, useEffect, useMemo, useState, Suspense } from "react";
...
function AuthPageContent() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-zinc-100 tracking-tight">Market Signals</h1>
          <p className="text-sm text-zinc-400 mt-1">Premium Trading Intelligence</p>
        </div>

        {/* Main Card */}
        <div className="rounded-3xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-xl p-8 shadow-2xl shadow-black/50">
          {/* Mode Toggle */}
          ...
          <div className="mb-6">
            ...
            {mode === "signup" && subscriptionTier !== "trial" && (
              <div className="mt-3 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-400/20 to-orange-500/20 border border-amber-500/30 px-3 py-1.5">
          ...
          {configLoaded && sheetsConfigured === false ? (
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-100 mb-6">
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <div>
                  <p className="font-medium text-amber-100">Local Storage Mode</p>
                  <p className="text-xs text-amber-200/80 mt-1">Accounts are saved locally. Configure Google Sheets for cloud backup.</p>
                </div>
              </div>
            </div>
          ) : null}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {mode === "signup" && (
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-zinc-700 bg-zinc-800/50 px-4 py-3 text-base text-zinc-100 placeholder-zinc-500 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition-all"
                />
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 px-6 py-3.5 text-sm font-semibold text-zinc-950 transition-all hover:from-amber-500 hover:to-orange-600 disabled:cursor-not-allowed disabled:opacity-60 shadow-lg shadow-amber-500/20"
            >
              {busy ? (
