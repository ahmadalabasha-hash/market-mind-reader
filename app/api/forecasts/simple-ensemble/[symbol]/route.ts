...
export async function GET(
  ...
  try {
    ...
    const filePath = path.join(process.cwd(), 'timesfm', 'simple_ensemble_forecasts', `${symbol.toLowerCase()}_simple_ensemble.json`);
    
    if (!fs.existsSync(filePath)) {
      return NextResponse.json(
        { error: 'Simple ensemble forecast not found for this symbol' },
        { status: 404 }
      );
    }
    ...
    console.error('Simple ensemble forecast error:', error);
    return NextResponse.json(
      { error: 'Failed to load simple ensemble forecast' },
      { status: 500 }
    );
  }
}
